...
import { expandEnvs } from '../src/expand.js'
...
describe('expandEnvs', () => {
  ...
  it('expands all keys except DOTENV_PUBLIC_KEY*', () => {
    expect(
      expandEnvs(parsedFixture, { type: 'all' }, fixturePrivateKey),
    ).toEqual({
      HELLO: 'hello-from-ci',
  ...
  it('skips both DOTENV_PUBLIC_KEY and DOTENV_PUBLIC_KEY_*', () => {
    ...
    expect(expandEnvs(parsed, { type: 'all' }, fixturePrivateKey)).toEqual({
      HELLO: 'plain',
    })
  })
  ...
  it('throws when named keys are missing from the env file', () => {
    expect(() =>
      expandEnvs(
        parsedFixture,
        { type: 'named', names: ['HELLO', 'MISSING'] },
        fixturePrivateKey,
      ),
    ).toThrowError('keys not found in env file: MISSING')
  })

  it('throws when there are no keys to expand', () => {
    expect(() =>
      expandEnvs({}, { type: 'all' }, fixturePrivateKey),
    ).toThrowError('env file contains no keys to expand')

    expect(() =>
      expandEnvs(
        { DOTENV_PUBLIC_KEY_CI: 'pub' },
        { type: 'all' },
        fixturePrivateKey,
      ),
    ).toThrowError('env file contains no keys to expand')
    ...
    expect(() =>
      expandEnvs(parsedFixture, { type: 'named', names: [] }, fixturePrivateKey),
    ).toThrowError('env file contains no keys to expand')
  })
