...
import { DecryptError, decrypt, isEncrypted, type EncryptedValue } from "../src/decrypt.js";
...
const wrongPrivateKey = "9c1ab41477004e68066129a8866887d316ba5d7177593dbc5e3026d6f64d32f8";
...
function expectDecryptError(fn: () => unknown, code: DecryptError["code"]): DecryptError {
  try {
    fn();
...
line 3`;
  ...
  it("decrypts the CI fixture values", () => {
    ...
    expect(decrypt(parsed.HELLO as EncryptedValue, fixturePrivateKey)).toBe("hello-from-ci");
    expect(decrypt(parsed.EXTRA as EncryptedValue, fixturePrivateKey)).toBe("should-not-leak");
    expect(decrypt(parsed.PLAIN as EncryptedValue, fixturePrivateKey)).toBe("visible");
  });
  ...
  it("throws MISSING_PRIVATE_KEY when the private key is empty", () => {
    ...
    expectDecryptError(() => decrypt(encryptedString, "   "), "MISSING_PRIVATE_KEY");
    expectDecryptError(() => decrypt(encryptedString, "\n,\t"), "MISSING_PRIVATE_KEY");
  });
  ...
  it("throws WRONG_PRIVATE_KEY when the key does not match", () => {
    expectDecryptError(() => decrypt(encryptValue("hello"), wrongPrivateKey), "WRONG_PRIVATE_KEY");
  });

  it("throws MALFORMED_ENCRYPTED_DATA for truncated ciphertext", () => {
    expectDecryptError(() => decrypt("encrypted:1234", privateKey), "MALFORMED_ENCRYPTED_DATA");
    expectDecryptError(() => decrypt("encrypted:", privateKey), "MALFORMED_ENCRYPTED_DATA");
  });
  ...
  it("tries comma-separated private keys until one works", () => {
    const encryptedString = encryptValue("hello");
    expect(decrypt(encryptedString, `${wrongPrivateKey},${privateKey}`)).toBe("hello");
    expect(decrypt(encryptedString, `  ${wrongPrivateKey} , ${privateKey} \n`)).toBe("hello");
  });
